"""
更换企业微信客服账号头像/名称
用法: python3 update_kf_avatar.py <图片路径> [open_kfid] [新名称]
      参数不传则交互选择
"""
...
def update_kf(token, open_kfid, media_id=None, name=None):
...
def main():
    ...
    if name is None:
        name = input("新名称（留空则不修改）: ").strip() or None

    update_kf(token, open_kfid, media_id=media_id, name=name)
